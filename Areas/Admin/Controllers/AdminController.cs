...
public class AdminController : Controller
{
    ...
    public async Task<IActionResult> CreateProduct([FromForm] Product product, [FromForm] IFormFile? image)
    {
        ...
        // Перевірка валідності JSON (додатковий захист)
        if (!string.IsNullOrWhiteSpace(product.Specifications))
        {
            try
            {
                System.Text.Json.JsonDocument.Parse(product.Specifications);
            }
            catch
            {
                ModelState.AddModelError("Specifications", "Невірний формат JSON.");
            }
        }
        ...
        if (image != null && image.Length > 0)
        {
            ...
            {
                ModelState.AddModelError("image", "Недозволений формат файлу.");
                ViewBag.Categories = await _context.Categories.ToListAsync();
                return View(product);
            }
            ...
            {
                ModelState.AddModelError("image", "Розмір файлу не повинен перевищувати 5MB.");
                ViewBag.Categories = await _context.Categories.ToListAsync();
                return View(product);
            }
